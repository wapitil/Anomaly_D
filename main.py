...
from yolov5.runtime.python.yolov5_det import YOLOv5Config, YOLOv5Detect

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))

PROJECT_ROOT = os.path.join("rdk_model_zoo")
DEFAULT_MODEL_PATH = os.path.join("yolov5n_tag_v7.0_detect_640x640_bayese_nv12.bin")
DEFAULT_TEST_IMAGE = os.path.join("yolov5/test_data/bus.jpg")
DEFAULT_RESULT_IMAGE = os.path.join("outputs", "result.jpg")
...
def main() -> None:
    ...
    labels = (
        file_io.load_class_names(args.label_file)
    ...
    boxes = np.array(
        [[x1, y1, x2, y2] for _, _, x1, y1, x2, y2 in results], dtype=np.float32
    )
