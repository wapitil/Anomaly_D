...
from utils import parse, find_first_usb_camera, draw_detection, class_id2name
import datetime
import ffmpeg
from network import sendDefect
import asyncio
...
def __start_detect():
    ...
    rtsp_url = parse['rtsp_url']
    if rtsp_url == '':
        logger.error("rtsp_url is empty")
        process = None
    else:
        process = (
            ffmpeg
    ...
    usb_id = find_first_usb_camera()
    cap = cv2.VideoCapture(usb_id)
    logger.info(f"usb_id = {usb_id}")
    model = YOLOv5_Detect(parse['model_path'], nc=int(parse['model_nc']))

    while True:
        _, img = cap.read()
        # 去掉img最下边的20%
        img = img[:int(img.shape[0]*0.75), :, :]
        ...
        if process is not None or len(ids) > 0:
            ...
            cv2.putText(out_img, current_time, position, font, font_scale, color, thickness)
...
def start_detect():
    thread = threading.Thread(target=__start_detect)
    thread.start()